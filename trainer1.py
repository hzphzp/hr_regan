...
class Trainer(object):
    ...
    def psnr(original, compared):
        ...
        compared = 255*(compared.numpy())
        mse = np.mean(np.square(original - compared),axis=[1,2,3])
        psnr = np.clip(
    ...
    def train(self):
        ...
        #print('ssssfffff')
            ...
            for g_step in range(500):
                ...
                l_const_AB = d(x_AB, x_B)
                l_const_BA = d(x_BA, x_A)
                ...
                l_g = l_gan_A + l_gan_B + 1000*(l_const_A + l_const_B+l_const_AB+l_const_BA) + (l_dsl_A + l_dsl_B)
            ...
            if step % self.log_step == 0:
                ...
                print("[{}/{}] l_dsl_A: {:.4f} l_dsl_B: {:.4f}". \
                      format(step, self.max_step, l_dsl_A.data[0], l_dsl_B.data[0]))
