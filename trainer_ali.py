...
class Trainer(object):
    ...
    def build_model(self):
        if self.dataset == 'toy':
            ...
            self.D_A = DiscriminatorCNN(
                    2*a_channel, 1, conv_dims, self.num_gpu)
    ...
    def train(self):
	    ...
	    #print('ssssfffff')
            ...
            x_Ac = torch.cat((x_A, x_AB),1)
            x_Bc = torch.cat((x_BA, x_B),1)

            x_ABA = self.G_BA(x_AB).detach()
            x_BAB = self.G_AB(x_BA).detach()

            if self.loss == "log_prob":
                l_d_A_real, l_d_A_fake = bce(self.D_A(x_Ac), real_tensor), bce(self.D_A(x_Bc), fake_tensor)
                ...
                l_d_A_real, l_d_A_fake = \
                    0.5 * torch.mean((self.D_A(x_Ac) - 1)**2), 0.5 * torch.mean((self.D_A(x_Bc))**2)
            ...
            for g_step in range(3):
                ...
                if self.loss == "log_prob":
                    l_gan_A = bce(self.D_A(x_Ac), fake_tensor)
                    l_gan_B = bce(self.D_A(x_Bc), real_tensor)
                elif self.loss == "least_square":
                    l_gan_A = 0.5 * torch.mean((self.D_A(x_Ac) - 0)**2)
                    l_gan_B = 0.5 * torch.mean((self.D_A(x_Bc) - 1)**2)
                else:
                    raise Exception("[!] Unkown loss type: {}".format(self.loss))

                l_g = l_gan_A + l_gan_B + 100*(l_const_A + l_const_B)
            ...
            if step % self.log_step == 0:
                ...
                print("[{}/{}] l_d_A_real: {:.4f} l_d_A_fake: {:.4f}". \
